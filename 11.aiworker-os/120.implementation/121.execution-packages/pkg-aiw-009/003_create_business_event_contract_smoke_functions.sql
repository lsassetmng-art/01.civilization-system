...
declare
  ...
  v_inbound_code text;
  v_inbound_note_text text;
  v_outbound_success boolean;
  v_outbound_code text;
  v_outbound_note_text text;
begin
  perform aiworker.fn_seed_business_event_contract_smoke_payloads();

  select r.success_flag, r.validation_result_code, r.note_text
    into v_inbound_success, v_inbound_code, v_inbound_note_text
  ...
  perform aiworker.fn_log_business_cross_boundary_smoke(
    ...
    coalesce(v_inbound_code, v_inbound_note_text),
    coalesce(p_executed_by, 'SYSTEM')
  );

  select r.success_flag, r.validation_result_code, r.note_text
    into v_outbound_success, v_outbound_code, v_outbound_note_text
    from aiworker.fn_validate_business_contract_payload_ref(
  ...
  perform aiworker.fn_log_business_cross_boundary_smoke(
    ...
    coalesce(v_outbound_code, v_outbound_note_text),
    coalesce(p_executed_by, 'SYSTEM')
  );
  ...
  return query
  values
    ('BIZ_INBOUND_REQUEST_EVENT_SMOKE', case when v_inbound_success then 'PASS' else 'FAIL' end, coalesce(v_inbound_code, v_inbound_note_text)),
    ('AIW_OUTBOUND_RESULT_EVENT_SMOKE', case when v_outbound_success then 'PASS' else 'FAIL' end, coalesce(v_outbound_code, v_outbound_note_text)),
