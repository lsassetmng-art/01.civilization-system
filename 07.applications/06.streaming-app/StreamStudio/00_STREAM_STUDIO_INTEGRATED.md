...
summary:
Defines the constitutional principles of StreamStudio,
...
purpose:
Defines the foundational principles of StreamStudio.

# 1. MISSION

StreamStudio is the official creator production and
publishing operations application for StreamingOS.

It exists to let creators and production teams manage:
- upload
- metadata
- thumbnail
- subtitle
...
1. StreamingOS is the source of truth.
2. StreamStudio is the creator operating surface.
3. Draft editing and canonical publication are separate.
4. Review is separate from approval.
5. Approval is separate from publication.
6. External push is downstream only.
7. External push success never replaces StreamingOS truth.
8. Rights and split basis must be auditable.
9. Mobile is light-operation-first.
10. Desktop and tablet are heavy-operation-first.
11. Multilingual support is first-class.
12. Multicurrency support is first-class.

# 3. TRUTH BOUNDARY

Canonical truth belongs to StreamingOS for:
- asset ownership
- publication truth
- approval trail truth
- monetization linkage truth
- rights and lifecycle truth

StreamStudio may hold:
- creator drafts
- creator UI state
- upload working state
- review work state
- split preparation state

# 4. EXTERNAL PLATFORM BOUNDARY

External platforms such as YouTube are treated as:
...
- source of truth
- ownership authority
- canonical publication authority for StreamingOS-native assets

# 5. GOVERNANCE PRINCIPLES

- Every critical action must remain reconstructible.
...
purpose:
Defines the formal application definition of StreamStudio.

# 1. DEFINITION

StreamStudio is a StreamingOS creator production,
publishing, commerce, and membership management application
operating under 07.applications/06.streaming-app.

It is not only an upload screen.
It is a creator operations cockpit that unifies:
- upload
- edit preparation
- review
- approval
...
- StreamingOS is the platform truth holder for creator asset and publication.
- StreamStudio is the creator-side production and commerce app.
- StreamWatch is the viewer-side watch app.
- Civilization Marketplace is the commerce distribution surface.
- External platforms are downstream publication targets.
...
- paid video price setting
- access type setting
- member-only release setting
- membership tier management
- membership benefit management
- member access rule management
- split setup
- fixed fee setup
- negotiation log
...
including creator workspace, upload intake,
...
purpose:
Defines policy requirements of StreamStudio.

# 1. RIGHTS CHECK POLICY

Before publication or sale, StreamStudio must support checks for:
- source ownership basis
- collaborator contribution basis
- thumbnail rights
- subtitle rights
- destination eligibility
- marketplace eligibility
- blocked internal-only material exclusion
- membership-only release rights compatibility

# 2. APPROVAL POLICY

Approval must remain explicit when:
- role policy requires approval
- rights risk exists
- publication is scheduled to external destination
- marketplace listing enters paid mode
- membership program becomes active
- split conflict exists
- policy override was requested

# 3. PRICING POLICY

- one-time price must be positive
- currency must be explicit
- free mode and paid mode must not coexist ambiguously on the same offer
- sale window must be valid
- membership-only mode must not silently inherit one-time purchase mode

# 4. MEMBERSHIP POLICY

- membership program activation may require review
- tier order must be unique
- benefit visibility must be explicit
- members-only content rules must be auditable
- early access expiration must be explicit
- program pause and end must preserve history

# 5. SPLIT POLICY

- percentage total must remain valid
- fixed fee currency must be explicit
- negative payout is forbidden
- overlapping active split versions are forbidden
- approval may be required for split activation
- paid listing may require split configuration before listing activation

# 6. PRIVACY POLICY

- private and limited drafts must not leak through search
- collaborator access must be scope-restricted
- negotiation logs may require reduced visibility
- member reference views must follow minimum necessary visibility
- connector credentials must remain isolated

# 7. POLICY FIXED STATEMENT

StreamStudio shall preserve explicit rights checks,
...
recommended_candidate_data_areas:
- streaming_creator_asset_area
- streaming_upload_queue_area
- streaming_metadata_draft_area
- streaming_thumbnail_area
- streaming_subtitle_area
- streaming_project_collaboration_area
- streaming_revision_area
- streaming_approval_area
- streaming_publish_setting_area
- streaming_marketplace_listing_area
- streaming_paid_video_offer_area
- streaming_membership_program_area
- streaming_membership_rule_area
- streaming_membership_reference_area
- streaming_external_push_area
- streaming_revenue_split_area
- streaming_settlement_reference_area
- streaming_connector_audit_area
...
phase_1_mvp_creator_publish_base:
- upload
- metadata
- thumbnail
- subtitle base
...
- access mode binding
- listing validation
- commerce release control

phase_4_membership_management:
- membership program
- tier management
- benefit management
- members-only content rules
- member early-access control

phase_5_monetization_collaboration:
- collaborator scopes
- split editor
- negotiation logs
- settlement reference

phase_6_external_connector_extension:
- YouTube push
