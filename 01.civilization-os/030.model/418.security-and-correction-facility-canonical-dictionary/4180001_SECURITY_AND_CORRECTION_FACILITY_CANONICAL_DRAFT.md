# ============================================================
# SECURITY AND CORRECTION FACILITY CANONICAL DRAFT
# ============================================================

status: canonical_draft
source:
- 2860101_INITIAL_MILITARY_FACILITY_NAME_CANDIDATE_MODEL.md
- 2860102_INITIAL_SECURITY_FACILITY_NAME_CANDIDATE_MODEL.md
- 2860103_INITIAL_CORRECTION_FACILITY_NAME_CANDIDATE_MODEL.md
- 2860104_MAJOR_CITY_SECURITY_AND_CORRECTION_ASSIGNMENT_MODEL.md
prepared_by: Zero

# ============================================================
# ENTRY CANDIDATES
# ============================================================

- code: ASIC_EXECUTIVE_SECURITY_OFFICE
  display_name: ASIC Executive Security Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ARSENAL_INTERNAL_GUARD_OFFICE
  display_name: Arsenal Internal Guard Office
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: AURELIA_CORRECTION_AUTHORITY_CENTRAL_WARD
  display_name: Aurelia Correction Authority Central Ward
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: AURELIA_NORTHERN_FLEET_COMMAND
  display_name: Aurelia Northern Fleet Command
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: AURELIA_SACRED_ORDER_OFFICE
  display_name: Aurelia Sacred Order Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: AURELIA_SACRED_SECURITY_LEGION_HALL
  display_name: Aurelia Sacred Security Legion Hall
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: BAD_DRUNK_NIGHT_SAFETY_OFFICE
  display_name: Bad Drunk Night Safety Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: BARRACKS_SURVEILLANCE_BUREAU
  display_name: Barracks Surveillance Bureau
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: BELIEVER_CONDUCT_AUTHORITY
  display_name: Believer Conduct Authority
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: BORDER_SECURITY_CONTAINMENT_FACILITY
  display_name: Border Security Containment Facility
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: CENTER_CIVIC_CORRECTION_CENTER
  display_name: Center Civic Correction Center
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: CENTER_COMMERCIAL_SECURITY_BUREAU
  display_name: Center Commercial Security Bureau
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: CONCORDIA_CONDUCT_MEDIATION_HOUSE
  display_name: Concordia Conduct Mediation House
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: CONCORDIA_MEDIATION_SECURITY_HOUSE
  display_name: Concordia Mediation Security House
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: DRONE_SECURITY_MONITORING_CENTER
  display_name: Drone Security Monitoring Center
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ELEWA_COASTAL_WATCH_OFFICE
  display_name: Elewa Coastal Watch Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: FIFTH_ARSENAL_COMMAND
  display_name: Fifth Arsenal Command
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: FIRST_PORT_BASE_NAVAL_COMMAND
  display_name: First Port Base Naval Command
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: FOURTH_PORT_BASE_NAVAL_COMMAND
  display_name: Fourth Port Base Naval Command
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: FREE_CITIES_UNION_USES_DISTRIBUTED_CIVIC_DEFENSE
  display_name: Free Cities Union uses distributed civic defense
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: FREEDAL_HARBOR_SAFETY_GUILD
  display_name: Freedal Harbor Safety Guild
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: FREEDAL_HARBOR_WATCH_OFFICE
  display_name: Freedal Harbor Watch Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: GRAND_SECURITY_LIAISON_OFFICE
  display_name: Grand Security Liaison Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: INTERNAL_DISCIPLINE_CORRECTION_BLOCK
  display_name: Internal Discipline Correction Block
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ISLAND_CONDUCT_RECOVERY_HOUSE
  display_name: Island Conduct Recovery House
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ISLAND_FERRY_SECURITY_HOUSE
  display_name: Island Ferry Security House
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ISLAND_MARITIME_WATCH_STATION
  display_name: Island Maritime Watch Station
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: IVANOV_INTERNAL_DISCIPLINE_BUREAU
  display_name: Ivanov Internal Discipline Bureau
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: IVANOV_NAVAL_PROJECTION_BASE
  display_name: Ivanov Naval Projection Base
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: KAMENGRAD_CORRECTION_FORTRESS
  display_name: Kamengrad Correction Fortress
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: LIBERTA_CIVIC_DEFENSE_OFFICE
  display_name: Liberta Civic Defense Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: LIBERTA_CIVIC_HOLDING_HALL
  display_name: Liberta Civic Holding Hall
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: LIBERTA_CIVIC_SAFETY_OFFICE
  display_name: Liberta Civic Safety Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: MIKHAIL_COASTAL_WAR_OFFICE
  display_name: Mikhail Coastal War Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: MOBILE_BASE_STRATEGIC_PLATFORM
  display_name: Mobile Base Strategic Platform
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: MOJA_CIVIC_HOLDING_HOUSE
  display_name: Moja Civic Holding House
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: MOJA_COASTAL_GUARD_POST
  display_name: Moja Coastal Guard Post
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: MOJA_HARBOR_SAFETY_OFFICE
  display_name: Moja Harbor Safety Office
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: NONDOMINA_COORDINATION_GUARD_HOUSE
  display_name: Nondomina Coordination Guard House
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: NORTH_PORT_SECURITY_COMMAND
  display_name: North Port Security Command
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: NORTH_PORT_TRADE_OFFENSE_DETENTION_HALL
  display_name: North Port Trade Offense Detention Hall
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: NORTH_PORT_TRADE_SECURITY_BUREAU
  display_name: North Port Trade Security Bureau
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: NORTH_TRAINING_COMMAND
  display_name: North Training Command
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: NOVA_RELIES_MORE_ON_ALLIANCE_BACKED_SECURITY
  display_name: Nova relies more on alliance-backed security
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ORPHEUS_CORRECTION_POSTURE_IS_LIGHTER_AND_MORE_LOCAL
  display_name: Orpheus correction posture is lighter and more local
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ORPHEUS_IS_NOT_A_FORTRESS_MILITARY_STATE
  display_name: Orpheus is not a fortress military state
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: POLITICAL_JUVENILE_DOCTRINAL_REFORMATION_SCHOOL
  display_name: Political Juvenile Doctrinal Reformation School
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: SOUTH_GATE_COMMERCIAL_SECURITY_POST
  display_name: South Gate Commercial Security Post
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: THIRD_BASE_CENTRAL_COMMAND
  display_name: Third Base Central Command
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: THIRD_BASE_INTERNAL_SECURITY_AUTHORITY
  display_name: Third Base Internal Security Authority
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: THIRD_BASE_MILITARY_DETENTION_HALL
  display_name: Third Base Military Detention Hall
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: TUBA_RITUAL_HARBOR_GUARD
  display_name: Tuba Ritual Harbor Guard
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: WEST_BORDER_DEFENSE_LINE_COMMAND
  display_name: West Border Defense Line Command
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: CORRECTION_IS_LIGHTER_AND_MORE_MEDIATION_ORIENTED
  display_name: correction is lighter and more mediation-oriented
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ウ_ノ港湾警備局
  display_name: ウーノ港湾警備局
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: トレ機械防衛基地
  display_name: トレ機械防衛基地
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ヘリオス中央治安局
  display_name: ヘリオス中央治安局
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ヘリオス中央矯正院
  display_name: ヘリオス中央矯正院
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ヘリオス外縁再生警備所
  display_name: ヘリオス外縁再生警備所
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ヘリオス東軍事港
  display_name: ヘリオス東軍事港
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: ヘリオス東防衛司令部
  display_name: ヘリオス東防衛司令部
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 京中央更生院
  display_name: 京中央更生院
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 京中央治安局
  display_name: 京中央治安局
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 外縁更生施設
  display_name: 外縁更生施設
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 成和交通規律矯正所
  display_name: 成和交通規律矯正所
  facility_type_code: CORRECTION
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 成和公共秩序局
  display_name: 成和公共秩序局
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 成和鉄道防護指令所
  display_name: 成和鉄道防護指令所
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 東加古奈沿岸防衛所
  display_name: 東加古奈沿岸防衛所
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 東加古奈港湾警備局
  display_name: 東加古奈港湾警備局
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 東方国境防衛隊司令部
  display_name: 東方国境防衛隊司令部
  facility_type_code: MILITARY
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 東部保全更生施設
  display_name: 東部保全更生施設
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 東部国防駐屯地
  display_name: 東部国防駐屯地
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 東部警戒補給所
  display_name: 東部警戒補給所
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

- code: 鉄央交通保安局
  display_name: 鉄央交通保安局
  facility_type_code: POLICE
  city_code: TBD
  state_code: TBD
  status: review_required

