...
exact_persistence_audit_domain: runtime_session
fixed_at: 20260417_174751

persistence_touchpoints:
- session_create_record
- heartbeat_update_record
- suspend_resume_record
- terminate_or_expire_record
- runtime_audit_trace
...
audit_rules:
- every terminal session path must emit trace evidence
- expire and terminate must remain distinguishable in storage
- host binding and session lineage must remain queryable

# EXACT ACCEPTANCE AND DONE GATE FIXATION

exact_acceptance_domain: runtime_session
fixed_at: 20260417_175108

acceptance_checklist:
- create_session from valid released input returns active or created path
- invalid released input returns blocked path
- heartbeat timeout returns expired with fixed terminal_reason
- terminate path returns terminated with fixed terminal_reason
- suspend and resume remain distinguishable in state and audit evidence
- every terminal path emits trace evidence

done_definition:
- session status enum is fixed
- terminal reason family is fixed
- request payloads are fixed
- response examples are fixed
- persistence touchpoints are fixed
- terminal evidence fields are fixed

implementation_gate:
- no implementation may collapse expired into terminated
- no implementation may terminate without terminal_reason
- no implementation may create session from unreleased input
