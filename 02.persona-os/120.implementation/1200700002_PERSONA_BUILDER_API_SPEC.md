...
- builder_api may orchestrate draft and validation flows
- builder_api must not expose canonical mutable truth directly to external OS
- publish and release flows must converge on snapshot issue, not direct truth export

release_surface_rule:
- builder-side publish result must end in snapshot issue path
- external consumption must attach to signed snapshot and event contract only
- builder_api responses must not imply direct external read authority over internal truth
