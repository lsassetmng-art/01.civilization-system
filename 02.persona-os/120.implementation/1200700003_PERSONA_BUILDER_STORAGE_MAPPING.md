...
storage_class_rule:
- builder draft storage is PersonaOS internal work-state storage
- builder storage must not be exposed as external read truth
- mutable draft identity must not be reused as immutable release artifact identity

lineage_rule:
- publishable draft -> snapshot issue -> signed external artifact
- direct draft exposure to external OS is forbidden
- storage mappings must preserve separation between mutable work state and immutable release artifact
