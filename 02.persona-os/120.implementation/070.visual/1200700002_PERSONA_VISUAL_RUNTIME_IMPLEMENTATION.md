...
exact_persistence_audit_domain: visual_runtime
fixed_at: 20260417_174751

persistence_touchpoints:
- visual_resolution_request_record
- visual_resolution_result_record
- fallback_resolution_record where applicable
- visual_audit_trace
...
- persona_id
- release_source_id
- visual_profile_id where resolved
- resolution_status
- fallback_used flag where applicable
- recorded_at

audit_rules:
- fallback_resolved must remain distinguishable from resolved
- missing binding blocks must leave evidence
- release lineage used for resolution must remain traceable

# EXACT ACCEPTANCE AND DONE GATE FIXATION

exact_acceptance_domain: visual_runtime
fixed_at: 20260417_175108

acceptance_checklist:
- released source resolves to resolved or fallback_resolved
- unreleased source returns blocked_unreleased
- missing binding returns blocked_missing_binding
- fallback usage remains explicit in status or evidence
- internal terminal failure remains distinct from domain block results

done_definition:
- resolution status family is fixed
...
implementation_gate:
- no implementation may hide fallback usage
- no implementation may map unreleased and missing-binding to same block result
- no implementation may use internal failure as domain block code
